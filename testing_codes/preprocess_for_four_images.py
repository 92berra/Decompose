...
DEFAULT_FONTS_IMAGE_DIR = os.path.join(SCRIPT_PATH, './FFG_1/s3/target-image-data')

DEFAULT_FFG_1 = os.path.join(SCRIPT_PATH, './FFG_2/s3/generated-image-data')
DEFAULT_FFG_2 = os.path.join(SCRIPT_PATH, './FFG_4/s3/generated-image-data')
DEFAULT_FFG_3 = os.path.join(SCRIPT_PATH, './FFG_5/s3/generated-image-data')
DEFAULT_FFG_4 = os.path.join(SCRIPT_PATH, './FFG_6/s3/generated-image-data')
DEFAULT_FFG_5 = os.path.join(SCRIPT_PATH, './FFG_7/s3/generated-image-data')
DEFAULT_FFG_6 = os.path.join(SCRIPT_PATH, './FFG_9/s3/generated-image-data')

DEFAULT_OUTPUT_DIR = os.path.join(SCRIPT_PATH, './combine-images/test3/style3')
...
def combine(src, src_path):
    ...
    elif args.b6_dir is None:
        raise Exception("missing b6_dir")
    ...
    for ext in [".png", ".jpg"]:
        sibling_path6 = os.path.join(args.b6_dir, basename + ext)
        if os.path.exists(sibling_path6):
            sibling6 = im.load(sibling_path6)
            break
    else:
        raise Exception("could not find sibling6 image for " + src_path)
    ...
    if sibling6.shape[2] == 1:
        sibling6 = im.grayscale_to_rgb(images=sibling6)
    ...
    if sibling6.shape[2] == 4:
        sibling6 = sibling6[:,:,:3]

    return np.concatenate([src, sibling1, sibling2, sibling3, sibling4, sibling5, sibling6], axis=1)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--b5_dir", type=str, default=DEFAULT_FFG_5,
                        help="path to folder containing B images of white characters for combine operation")
    parser.add_argument("--b6_dir", type=str, default=DEFAULT_FFG_6,
                        help="path to folder containing B images of white characters for combine operation")
