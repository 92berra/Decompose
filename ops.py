...
weight_init = tf.keras.initializers.VarianceScaling(scale=factor, mode=mode, distribution='uniform' if uniform else 'truncated_normal')
#weight_init = tf.compat.v1.layers.variance_scaling_initializer(factor=factor, mode=mode, uniform=uniform)
...
def discrim_conv(batch_input, out_channels, stride):
    padded_input = tf.pad(batch_input, [[0, 0], [1, 1], [1, 1], [0, 0]], mode="CONSTANT")
    return tf.keras.layers.Conv2D(filters=out_channels, kernel_size=4, strides=(stride, stride), padding="valid", kernel_initializer=tf.keras.initializers.RandomNormal(mean=0.0, stddev=0.02))(padded_input)
...
def batchnorm(inputs, training=True):
    return tf.keras.layers.BatchNormalization(axis=3, epsilon=1e-5, momentum=0.1, 
...
# tensorflow 2.13
def up_sample(x, scale_factor=2):
    _, h, w, _ = x.shape  # 더 이상 get_shape().as_list()를 사용하지 않습니다.
    new_size = [h * scale_factor, w * scale_factor]
